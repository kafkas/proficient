...
npx typedoc --out docs/$package_id/$version --entryPoints "$package_path/src/index.ts" --tsconfig "$package_path/src/tsconfig.json"

# Step 4: Update docs readme

ts-node scripts/updatePackageDocsIndexFile.ts $package_id
